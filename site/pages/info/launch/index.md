...
Audiences come here because the posts they were supposed to see on other platforms are visible again. The feed is built on a published, inspectable ranking algorithm — not an editorial hand on the scale. Every post carries a "why am I seeing this?" explanation: six numbers, one formula, no black box.

Discovery is split deliberately: half the feed is people you chose to follow, a third is high-quality voices from outside your graph, and a meaningful slice is reserved for small and new creators who would otherwise never surface. A reader who comes here actually finds the voices the legacy platforms hid.
...
- **Sovereign deployment.** Server Node lets a user run a piece of the network on their own PC. On-prem AI lets an organization run the entire moderation stack on hardware it owns. There is no master server to seize.

## Our Seven Promises

These are the promises we make to every customer, in priority order. Everything else supports this list.

1. **Free speech.** Lawful speech stays up. We do not silence customers because their views are unpopular with regulators, advertisers, or activist staff at incumbent platforms.
2. **We grow their audience.** Feed ranking, cross-network publishing, trust-based discovery, and Hero amplification are all built to surface suppressed voices.
3. **Protection from manufactured narratives.** Inorganic amplification is detected. Coordinated suppression is exposed. Authentic human signal outweighs manufactured consensus — wired into the algorithm, not just into the marketing.
4. **Transparent ranking.** Every customer can see why a post ranks where it ranks. No black-box editorial hand on the scale.
5. **Open-source code.** Audit it. Fork it. Run your own instance. Trust is earned through inspection, not asserted through marketing.
6. **Open-standard backbones.** Identity, content, and social graph are not locked to a single vendor — including us.
7. **Open content.** Content is open to be read, indexed, archived, mirrored, cross-posted, and federated. We make it easier — not harder — for a citizen's words to reach the largest possible authentic audience.
...
## Built So We Cannot Be Pressured

The infrastructure underwrites every promise above. Multi-region relay clusters mean there is no single jurisdiction that can take us down. Identity is a cryptographic keypair — math, not a row in our database. Media is content-addressed and portable. The WebSocket protocol is open Nostr, so any compliant client can read and post a customer's content. We earn the customer's trust every day by being replaceable, not by locking them in.

## Welcome

If you have been suspended, shadow-banned, demoted, or simply tired of speaking into a void engineered to swallow you — you are who we built this for.
